...
func TestVersionIsTheFreeze(t *testing.T) {
	...
	if Version != "0.12.0" {
		t.Fatalf("Version = %q, want 0.12.0 (pre-1.0, feature-complete)", Version)
	}
...
func TestIsMutatingPredicate(t *testing.T) {
	...
	for _, n := range []string{"bash", "write", "edit", "python", "scheduler", "plugins_reload", "delegate", "gpu_stats"} {
		if !r.isMutating(n) {
