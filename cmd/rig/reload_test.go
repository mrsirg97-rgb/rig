...
`)
	...
	if len(h.r.k.Middleware) != 8 {
		t.Fatalf("middleware = %d links, want the path boundary, the router, the cutoff link, the provenance rule, the allow-list, the bound, the round cap, and the result bound", len(h.r.k.Middleware))
	}
