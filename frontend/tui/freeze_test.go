...
func TestFreezeGate(t *testing.T) {
	...
	// the named reopening (SPEC_EVT 2a, the batch): loop/ and kernel.go
	// change under a spec'd deliverable; the re-freeze PR after the merge
	// deletes this function and the gate measures the new bytes.
	reopened := func(p string) bool {
		return p == "loop" || strings.HasPrefix(p, "loop/") || p == "kernel.go"
	}
