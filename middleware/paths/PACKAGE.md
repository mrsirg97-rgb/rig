...
The path boundary: one link in the root's chain that expands a bare `~`
and a leading `~/…` in a tool call's path-shaped arguments before any
tool sees them. A model's path habits are half shell and half API;
...
so every tool inherits it and nothing drifts. 1.1.4 narrows it to the
shell's most common form — bare `~`, leading `~/` — and leaves `~user`
and a mid-path `~` as given.
...
- `Expand(p)`: `~` and `~/…` are `os.UserHomeDir` (`$HOME`; the scratch
  home inside a jail, the home the session prompt names). `~user`, an
