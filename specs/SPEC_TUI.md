...
door, `WithTitle(rows, tagline)` — the embedder's rows replace the rig
letters and the tagline renders under them, default the rig rows with
no tagline, and the ascii fallback prints the first row plain); the
session id under it (its first twelve
