...
// arm's shadow), meta (versions and the id-minting counter). Lift's
// four-tag grammar is the language. Nullable columns are pointers.
// Supersession pairs a
...
// transaction — never reused (the AUTOINCREMENT rule, kept by minting
// since the camera emits plain PRIMARY KEY).
type Memory struct {
	...
	ContentSha256      string  `alias:"name=content_sha256,nullable=false"`
}

// table:"trigrams"
//
// Composite primary (memory_id + gram): the natural key of one shadow
// row, enforced by the database. The link column generates the
// bulk-delete accessor the prune's bookkeeping rides.
type Trigram struct {
...
// what the camera cannot emit. Comment-only fragments drop; order is
// preserved.
