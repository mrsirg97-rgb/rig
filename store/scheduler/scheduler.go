// Package scheduler is the background-jobs store: pane's scheduler design,
// Go over the generated substrate. SPEC_STATE's "### scheduler" section is
// the spec; pane's SCHEDULER_SPEC and scheduler/ carry the voice and the
// named cases.
//
...
// and run history survives compaction, which pane's event-args-only shape
...
// alone. Pane's CHECK-style invariants are not expressible in the camera
// and are enforced in Go where pane also enforces them; pane's schema
// carries no indexes, so there is no extra.sql here.
