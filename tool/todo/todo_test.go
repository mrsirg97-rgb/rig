...
// --- SPEC_UX 1: the add verb at the tool's door ---

func TestAddAppendsATask(t *testing.T) {
	tool := todoapi.New(newDB(t))
	if _, err := exec(t, tool, context.Background(), map[string]any{"action": "create", "tasks": []any{map[string]any{"text": "existing"}}}); err != nil {
		t.Fatal(err)
	}
	reply, err := exec(t, tool, context.Background(), map[string]any{"action": "add", "text": "wire the guard"})
	if err != nil {
		t.Fatalf("add: %v", err)
	}
	if !strings.Contains(reply, "t2 added: wire the guard") {
		t.Fatalf("the add reply names the minted id and the text:\n%s", reply)
	}
	if !strings.Contains(reply, "t1 [ ] existing") {
		t.Fatalf("add must not touch the existing queue:\n%s", reply)
	}
}

func TestAddRefusesMissingText(t *testing.T) {
	tool := todoapi.New(newDB(t))
	if _, err := exec(t, tool, context.Background(), map[string]any{"action": "add"}); err == nil {
		t.Fatal("add without text succeeded")
	} else if want := "action 'add' requires text"; err.Error() != want {
		t.Errorf("voice:\n%q", err.Error())
	}
}

// TestAddSurvivesAForeignClaim is the parallel-session property (SPEC_UX 1):
// another session's in-flight task stands through my add — the verb that
// exists because two sessions share one queue.
func TestAddSurvivesAForeignClaim(t *testing.T) {
	db := newDB(t)
	tool := todoapi.New(db)
	sessA := core.NewSession()
	sessB := core.NewSession()
	if _, err := exec(t, tool, core.WithSession(context.Background(), sessA),
		map[string]any{"action": "create", "tasks": []any{map[string]any{"text": "in flight"}}}); err != nil {
		t.Fatal(err)
	}
	if _, err := exec(t, tool, core.WithSession(context.Background(), sessA), map[string]any{"action": "start", "id": "t1"}); err != nil {
		t.Fatal(err)
	}
	reply, err := exec(t, tool, core.WithSession(context.Background(), sessB), map[string]any{"action": "add", "text": "my task"})
	if err != nil {
		t.Fatalf("add over a foreign claim must not refuse: %v", err)
	}
	if !strings.Contains(reply, "t1 [~] in flight") || !strings.Contains(reply, "claimed by") {
		t.Fatalf("the foreign claim stands through the add:\n%s", reply)
	}
	if !strings.Contains(reply, "t2 added: my task") {
		t.Fatalf("the add lands:\n%s", reply)
	}
}
