// Package web provides the web tools: web_search (the local SearXNG
...
// DefaultSearXNG is the SearXNG root: the web-tools compose
// (~/docker/web-tools) publishes SearXNG on loopback :8888.
const DefaultSearXNG = "http://127.0.0.1:8888"

// DefaultProxy is the egress proxy: the same compose's
...
// kernel's venv), then PATH. Empty when neither has
